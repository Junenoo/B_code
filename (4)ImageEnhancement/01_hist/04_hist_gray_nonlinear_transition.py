...
def gammaTranslation(img, c=1.0, v=1.0, b=0.0):
    ...
    伽马变换是对图像灰度值进行的非线性操作，使输出图像灰度值与输入图像灰度值呈指数的关系，
    目的就是用来图像增强，提升暗部细节，通过非线性变换，让图像从曝光强度的线性响应变得更加接近人眼感受的响应
    :param img: 原图
...
if __name__ == '__main__':
    ...
    lily02 = cv2.imread('../../../data/lily.png',0)
    ...
    plt.subplot(341)
    plt.title('gray_lily')
    plt.hist(cv2.cvtColor(lily, cv2.COLOR_BGR2GRAY).ravel(), bins=256)
    plt.subplot(342)
    plt.title('gray_nonlinearTranslation')
    ...
    plt.subplot(343)
    plt.title('gray_log')
    plt.hist(cv2.cvtColor(log, cv2.COLOR_BGR2GRAY).ravel(), bins=256)
    plt.subplot(344)
    plt.title('gray_logTranslation')
    ...
    # 灰度直方图
    plt.subplot(345)
    plt.title('gray_gamma01')
    plt.hist(cv2.cvtColor(gamma, cv2.COLOR_BGR2GRAY).ravel(), bins=256)
    plt.subplot(346)
    plt.title('gray_gamma01_res')
    ...
    # 灰度直方图
    plt.subplot(347)
    plt.title('gray_gamma02')
    plt.hist(cv2.cvtColor(gamma02, cv2.COLOR_BGR2GRAY).ravel(), bins=256)
    plt.subplot(348)
    plt.title('gray_gamma02_res')
    plt.hist(res04.ravel(), bins=256)

    res05 = gammaTranslation(gamma03, v=0.3)
    cv2.imshow('gamma03_res', res05)
    plt.subplot(349)
    plt.title('gray_gamma03')
    plt.hist(cv2.cvtColor(gamma03, cv2.COLOR_BGR2GRAY).ravel(), bins=256)
    plt.subplot(3, 4, 10)
    plt.title('gray_gamma03_res')
    ...
    plt.subplot(3, 4, 11)
    plt.title('gray_mi')
    plt.hist(cv2.cvtColor(pl, cv2.COLOR_BGR2GRAY).ravel(), bins=256)
    plt.subplot(3, 4, 12)
    plt.title('piecewise_linear01')
    plt.hist(res06.ravel(), bins=256)
